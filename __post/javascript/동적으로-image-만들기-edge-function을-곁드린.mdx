...
Vercel은 왜 기존 og-image generator를 새로운 툴인 satori와 함께 공개했을까요? 그리고 어떤 좋은 점이 있을까요?
...
정적 소셜 이미지를 만들고 공유하는 것은 어진 않지만 즉시 계산되고 실시간으로 생성해야 하는 **동적 이미지**를 처리하는 것에는 한계가 있었습니다.

그래서 4년 전 Vercel은 동적으로 이미지를 생성할 수 있는 Open Graph Image Generator를 릴리즈했습니다.
