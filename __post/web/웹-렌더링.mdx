...
> https://developers.google.com/web/updates/2019/02/rendering-on-the-web 번역 및 정리

개발자는 아키텍처를 구상하고 선택할 때 많은 조건에 의해서 결정합니다. 그 조건이 다양하고 많은 지표에 의해 정해집니다.

웹 개발자는 더 나은 렌더링 방식을 사용하여 사용자에게 서비스를 제공하려고 합니다. 그리고 아키텍처는 결정을 한 번 하면 바꾸기 어렵습니다.

늘 그렇지만 개발의 세계는 늘 빠르게 움직이고 성장하고 있습니다.
최근에 react 18 rc 버전이 나오면서 많은 SSR 프레임워크들은 발 빠르게 준비하고 있습니다.

렌더링 방식은 다양하고 각 렌더링 방식은 생각보다 어렵습니다. 각 접근법에 대해 확실히 이해하기 위해 이번에 정리할 겸 작성해보려고 합니다.
...
아키텍처를 더 잘 이해하고 선택하기 위해서 각 접근법에 대해 확실하게 이해하기 위해 용어를 정확하게 아는 것도 중요합니다.

- **SSR**: Server-Side Rendering(서버 측 렌더링) - 서버에서 생성된 HTML로 렌더링 합니다.
- **CSR**: Client-Side Rendering(클라이언트 측 렌더링) - 브라우저에서 렌더링합니다. 일반적으로 DOM을 사용합니다.
- **(Re)hydration**: 클라이언트가 서버에서 렌더링한 HTML을 렌더링 후 DOM트리와 상태등 자바스크립트를 통한 렌더링으로 재사용하기 위해 컴포넌트 트리를 재 구성하는 것을 말합니다.
- **Prerendering**: 빌드 타임에 애플리케이션을 실행하여 초기 상태를 정적 HTML로 캡쳐합니다.

조합하여 사용되기도 하며 더 세분화하면 다양한 렌더링 방식이 있습니다.
(Static SSR, SSG, SSR with hydration, CSR with prerendering, Streaming...)

## 왜 사용자 경험이 중요한가?

> Slow-loading pages are frustrating for users, who can't/won't use the site, and bad for developers, who forfeit those users.
...
> 서버사이드 렌더링은 사용자 요청에 대한 응답으로 페이지에 대한 전체 HTML을 생성하고 난 뒤 응답됩니다. 이렇게 하면 브라우저에서 응답을 받기 전에 다루어 지기 때문에 클라이언트에서 데이터를 위한 fetch 등 추가적인 요청을 피할 수 있습니다.

JSP, ASP, thymeleaf 등의 언어를 사용해보셨다면 가장 익숙한 유형입니다.
별도의 서버가 나눠져있지 않고 백엔드서버를 통해 모든 것이 이루어집니다.

![jsp-v2](https://upload.wikimedia.org/wikipedia/commons/7/72/JSP_Model_2.svg)

API 서버가 분리가 따로 필요 없기 단일 서버로 구현이 가능합니다.

SPA구조가 아닌 MPA 구조에 적합하고 모든 UI 요소를 미리 받아온 후, 필요한 데이터(주로 JSON)만을 요청하여(ajax) 갱신함으로써, 반응성을 빠르게 하는데에 있습니다.

말그대로 MPA 구조에 사용되기 떄문에 화면 전환(url path전환)의 경우 서버에 반드시 페이지를 만들어 달라고 요청을 해야 되기 때문에 아무리 빠르게 진행되더라도 사용자 경험은 좋지 않습니다.(화면 깜빡임 등)

그렇다고 SPA를 하지 못한다는 것은 아니지만 매 요청시에 모든 페이지에 해당하는 내용을 렌더링해야 되기 때문에 높은 서버 부담, 큰 초기 전송량이라는 단점을 그대로 가지고 있기 때문에 권장하지 않습니다.

### 특징

SSR의 장점은 빠른 FCP(First Contentrul Paint) 입니다.

또한 서버에서 페이지 렌더링에 필요한 많은 자바스크립트 코드를 클라이언트로 보내지 않아도 되므로 TTI(Time To Interactive)가 빨라집니다.
...
SEO에서도 검색엔진이 사이트를 호출 시에 완성된 메타 데이터를 크롤링할 수 있기 때문에 CSR보다 뛰어납니다.(검색 엔진 봇이 JS 사용을 허용하는지 여부와 상관없이 제공할 수 있습니다.)

SSR을 사용하면 사용자는 사이트를 이용하기 전에 자바스크립트가 처리되기를 기다리지 않아도 됩니다. third-party 자바스크립트를 부득이하게 사용해야 할 경우(ex, google ad)에도 메인 자바스크립트 비용을 줄이게 되므로 결국은 더 이득이 될 수 있습니다.
그러나 이런 경우 서버에서 페이지를 생성하는 데 시간이 더 걸릴 수 있다는 점을 알고 있어야 합니다.

애플리케이션이 SSR에 크게 의존하고 있는지 아닌지 충분히 생각할 필요가 있습니다.
SSR, CSR은 다른 곳에 서 있는 기술이 아닙니다. 일부 페이지에서는 SSR을 사용하고 다른 페이지에서는 사용하지 않아도 됨을 기억하는 것이 중요합니다.
...
- FCP, TTI가 CSR에 비해 더 빠릅니다.
- TTFB는 HTML 파일을 모두 완성한 뒤에 받게 되기 때문에 다른 방식에 비해 느립니다.
- SEO가 CSR 비해 검색 봇이 제약없이 크롤링이 가능하여 더 좋습니다.
...
concurrent, RSC 관련해서는 공부하며 차차 글을 올릴 생각입니다.
...
**정적 렌더링된 페이지는 많은 client-side 자바스크립트를 실행할 필요 없이 interacitve 합니다. 반면에 pre-rendering 페이지는 client에서 boot(자바스크립트를 통해)해야만 interactive가 되는 SPA의 페이지 FCP를 향상시킵니다.**

자바스크립트를 비활성한다면 차이는 확실하게 들어납니다. 정적 렌더링 페이지의 경우 대부분 interactive 합니다. 그러나 pre-rendering 페이지는 링크나 몇 가지 default action을 제외하곤 비활성상태가 됩니다.
...
SSR의 동적인 특성은 [상당한 컴퓨팅 오버헤드](https://medium.com/airbnb-engineering/operationalizing-node-js-for-server-side-rendering-c5ba718acfc9)가 발생할 수 있습니다. 많은 SSR 솔루션은 early flush 되지 않습니다. 그래서 TTFB를 지연시키거나 전송되는 데이터가 두 배가 될 수 있습니다.(예: 클라이언트에서 자바스크립트에서 사용되는 inlined 상태)
...
> CSR은 자바스크립트를 사용하여 브라우저에서 직접 페이지를 렌더링하는 것을 의미합니다. 모든 로직, fetch, templating, routing을 서버가 아닌 클라이언트에서 처리합니다.
...
전체 페이지 load나 re-load와 같은 네비게이션 요청은 서버에 의해서 처리하고,
렌더링에 필요한 자바스크립트, 데이터를 클라이언트에서 처리하도록 합니다.
...
그러나 희망은 있습니다. 단기적으로는 캐시 가능성이 높은 콘텐츠에만 SSR을 사용하면 TTFB 지연을 줄여 pre-rendering과 유사한 결과를 얻을 수 있습니다.
...
> [react 18에선 renderToNodeStream이 depreacted되고 renderToPipeableStream 사용을 권장합니다.](https://github.com/reactwg/react-18/discussions/22) > ![renderToPipeableStream](https://files.seonest.net/images/qnfqnfqnf/post/172/image.png)

점진적 hydration도 눈여겨볼 가치가 있으며 React가 [연구](https://github.com/facebook/react/pull/14717) 해왔습니다. concurrent 모드가 곧 릴리즈 될 것으로 예상되며 `<Suspense fallback={...}>` API를 이용하여 점진적 hydration을 처리하도록 합니다.
...
| :------------------------------------------------------------------------------------------------------------: |
| _[이미지 출처](https://developers.google.com/web/updates/images/2019/02/rendering-on-the-web/infographic.png)_ |

최근엔 SPA가 고민없이 사용하게 되고, 사용하지 않으면 큰일이 나는 것처럼 인식하는 경우도 많이 있었습니다.

SSR 솔루션을 사용하면 모든 문제가 풀릴 것이라고 생각했던 경우도 많았던 것 같습니다.

이 글을 작성하며 느꼈던 점은 렌더링에 관한 다양한 방법이나 솔루션이 모든 걸 해결해주리라는 은총알처럼 인식하고 있지 않은지 반성하게 됩니다. 또한 A가 B보다 반드시 좋다라는 편협한 시각을 가지고 있었던 건 아닐지 반성하게 됩니다.

대부분의 경우도 마찬가지지만 결정할 때 100% 만족하는 경우는 많이 없다는 것입니다.
렌더링 아키텍쳐를 선택할 때도 마찬가지인 것 같습니다. 정적 렌더링이나 서버 렌더링을 사용하여 90% 만족을 달성할 수 있다면 그게 정답이 될 수 있습니다.

이 글이 모든 걸 다 이해하고 쓰지도 않았지만, 조금은 이해를 해보려고 노력했던 것 같습니다.
앞으로도 이 주제와 관련한 글을 써야겠다는 생각이 듭니다.
