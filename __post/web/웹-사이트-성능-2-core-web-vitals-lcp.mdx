...
> 이번 포스트에서는 core web vitals와 그 중 하나인 LCP애 대해서 알아보도록 하겠습니다.

문제 해결을 위해서는 문제 정의가 우선되어야 합니다.

웹 사이트 성능 개선을 위해서는 성능 측정이 우선 되어야 합니다.

다양한 웹 사이트 성능 측정 도구들이 많이 있습니다.
...
- PageSpeed Insights는 오직 성능 지표만을 측정합니다. 반면에 Lighthouse는 웹 사이트의 다른 측면을 포함해서 검사합니다.(SEO, 접근성, PWA 등등)
- PageSpeed는 Lab 데이터와 실제 데이터의 "combo"를 사용합니다. 반면에 Lighthouse는 오직 Lab 데이터만 사용합니다.
...
- 웹사이트 방문자들이 실제 경험한 로딩 속도의 정확한 보고서가 필요할 때
- 웹사이트 성능을 분석하기 위해 크롬 개발자 도구를 사용하는 "Big Fan"이 아닐 때

### Lighthouse는 언제 사용해야 할까?

- 프로그래밍 방식으로 검사가 필요할 때
- 사이트 로딩 시간 뿐만아니라 사이트의 다양한 측면에서 평가가 필요할 때
- 본인 시스템에 Lighthouse API를 사용할 때

> 예를 들어 Lighthouse API를 사용하여 성능, SEO 표준, 접근성 등 기준을 충족하지 않을 때는 릴리스를 하지 않도록 막을 수 있습니다.

### 결론은

> Google PageSpeed는 Lighthouse에서 생성된 정보를 사용하여 실제 데이터로 정보를 보강합니다. Lighthouse는 성능 지표 뿐만 아니라 다양한 지표를 제공합니다. 웹사이트의 로딩 시간을 측정뿐만 아니라 그 이상을 확인입니다.

사이트를 테스트하는데 필요한 정보나 지표에 따라 선택하여 사용하면 될 것 같습니다.
...
결과적으로 PageSpeed Ingishts와 core web vitals를 이용해 성능 측면에서 약 3배정도 향상이 되었다고 하는데 어떤 개선을 통해 3배 개선이 되었는지 확인해보면 좋을 것 같습니다.
(기존에도 많은 개선을 통해서 운영을 했을 텐데... 대단한 느낌이랄까)

### 개요

성능 개선 분야는 [참으로 복잡](https://www.youtube.com/watch?v=ctavZT87syI)합니다(라고 여기서도 적혀있는데 실제로도 분석하기에 엄청 복잡한 것 같습니다.)

> 위 링크는 2019년 performance.now()라는 컨퍼런스에서 크롬 모니터링 성능 지표팀에서 많은 일을 해온 Aniie Sullivan의 성능 지표에 관한 강연입니다)
>
> 성능 지표에 왜 썼는지, 그리고 그것을 통해 어떻게 측정을 하는지에 대한 이야기들과 개선에 대한 내용을 말합니다.
...
구글에서는 core web vital을 기준으로 모니터링 시스템과 내부 토의를 진행하였습니다.
...
**이것이 가장 중요한 부분입니다.
HTTP응답이 오기전까지 (TTFB: Time To First Byte) 아무 일도 일어나지 않기 때문입니다.**
...
구글에서는 web vital이라고 하는 사용자 경험 품질 최적화 지침을 관리합니다.

그중 core web vitals는 가장 중심적이며 사용자 경험의 고유한 측면을 나타냅니다.

각 성능은 "양호(good)", "개선 필요(needs improvement)" 또는 "나쁨(poor)"으로 분류하는 관련 지표가 있습니다.

> 추가적으로 페이지 또는 사이트의 전반적인 성능을 분류하기 위해 해당 페이지 또는 사이트에 대한 모든 페이지 조회수의 75번째 백분위수 값을 사용합니다. 즉, 사이트에 대한 페이지 조회수의 75% 이상이 "양호"를 충족하는 경우 해당 사이트는 해당 메트릭에 대해 "양호"한 성능을 보이는 것으로 간주됩니다

2020년 core web vitals는 다음과 같습니다.

- LCP: Largest Contentful Paint
  가장 큰 시각 컨텐츠가 그려지는 시간
  인지된 로드 속도를 측정하고 페이지 로드 타임라인에서 페이지의 메인
- FID: First Input Delay
  사용자가 사이트에서 첫 상호작용에 반응되는 시간
  응답성을 측정하고 사용자가 페이지와 처음 상호 작용을 시도할 때 느끼는 경험을 수량화
- CLS: Cumulative Layout Shift
  사용자 경험을 방해할 수 있는 예상치 못한 레이아웃에 대한 정량적 지표
  시각적 안정성을 측정하고 시각적 페이지 콘텐츠의 예기치 못한 레이아웃 이동을 수치화하여 표현
...
core web vitals는 우수한 사용자 경험을 이해하고 제공하는데 중요한 지표이지만 다른 중요 지표 또한 존재합니다.
...
web vitals, core web vitals는 개발자가 웹 전반에 대한 경험 품질을 측정하기 위해 현재 사용할 수 있는 최상의 신호를 나타내지만 이러한 신호는 완벽하지 않으며 향후 개선 또는 추가가 이루어질 수 있습니다.

그렇지만 정의와 임계값이 안정적이고, 예측 가능한 주기로 업데이트가 이루어질 것입니다.
...
페이지의 메인 콘텐츠(라고 예상되는)가 화면에서 렌더링되는데 걸린 시간으로 사용할 수 있습니다.

LCP 측정에서 Poor가 발생하는 일반적인 이유는 다음과 같습니다.
...
브라우저가 서버에서 콘텐츠를 받는데 오래 걸린다면 무엇이든 렌더링하는데 더 오래 걸립니다. 더 빠른 서버 응답 시간은 LCP를 포함한 모든 단일 페이지 로드를 직접적으로 향상시킵니다.

가장 먼저 해야할 일은 서버에서 콘텐츠를 처리하는 방법과 위치를 개선하는 것입니다.

TTFB(Time To First Byte, 최초 바이트까지의 시간)으로 서버 응답 시간을 측정하세요.
다음과 같은 방식으로 TTFB를 개선할 수 있습니다.

- 서버 최적화
- CDN 사용
- Asset 캐시
- HTML 페이지 cache-first 제공
- establish thrid-party connections early
- use signed exchange

#### 서버 최적화

서버에서 상당한 시간이 걸리는 쿼리가 실행되고 있나요?
아니면 서버에서 복잡한 작업으로 인해 페이지 콘텐츠를 반환하는 프로세스가 지연되나요?
서버 코드 효율성을 분석하고 개선하면 브라우저가 데이터를 수신하는데 걸리는 시간이 빨라집니다.
...
thrid-party 서버 요청은 특히 페이지에 중요한 콘텐츠를 표시해야 하는 경우 LCP에 영향을 줄 수 있습니다. (예를 들어 웹폰트 등)
...
UI 프레임워크를 사용하거나 CSS-in-JS등 전처리기를 사용한다면 거기에서 제공하는 critical-css 옵션들을 사용해야되는 경우도 있습니다. 그러나 기본적인 동작방식을 이해한다면 적용하는데도 조금 더 수월하게 진행할 수 있을 것 같습니다.
...
필요한 최소한의 Javscript를 다운로드하여 사용자에게 제공합니다. 차단하는 Javascripe의 양을 줄이면 렌더링 속도가 빨라지고 결과적으로 LCP가 향상됩니다.
...
> 파일을 압축하면 성능이 크게 향상될 수 있지만 직접 수행할 필요는 거의 없습니다. 많은 플랫폼, CDN, reverse proxy 서버는 기본적으로 압축방식을 쉽게 구성할 수 있도록 합니다. 자체적으로 압축하기 전에 압축이 이미 지원되는지 확인하고 진행하면 좋을 것 같습니다.
...
webpack, parcel, rollup 과 같은 인기 있는 모듈 번들러는 dynamic imports를 사용하여 번들을 분할 할 수 있습니다.(code splitting)
...
webpack의 [SplitChunksPlugin](https://webpack.js.org/plugins/split-chunks-plugin/)이 이러한 작업을 어떻게 도울 수 있는 자세히 볼 수 잇습니다.

클라이언트 측 프레임워크를 사용할 때 route 또는 component에서 분할하는 것은 애플리케이션을 lazy loading하는 심플한 방법입니다.
...
> 재미있는 사실: Lebab은 Babel과 반대되는 작업을 수행하는 별도의 라이브러리입니다. 이전 코드를 새로운 구문으로 변환합니다.
...
> block-level 요소
> https://developer.mozilla.org/ko/docs/Web/HTML/Block-level_elements > `<p>` `<div>` `<h1>~<h6>`등 display: block 요소
> inline-level 요소: `<span>` `<a>` 등 display: inline 요소
...
아래 이미지는 위 이미지보다 40% 더 작지만 거의 동일하게 보입니다.
(20kb vs 12kb)
...
> Next.js에서는 SSG라는 이름으로 pre-rendering을 제공합니다.
...
> 클라이언트 렌더링
> ![client-rendering](https://developers.google.com/web/updates/images/2019/02/rendering-on-the-web/client-rendering-tti.png)

## 마무리하며

core web vitals 중 LCP와 관련된 지표와 개선사항들을 살펴보았습니다.
나머지 FID, CLS 최적화는 다음 포스트에서 다루어 볼 예정이지만 아마 겹치는 부분이 꽤 존재하지 않을까 싶습니다. 모두 연관이 되어있기 때문입니다.

다음 포스트에서는 FID부터 다뤄보도록 하겠습니다.
