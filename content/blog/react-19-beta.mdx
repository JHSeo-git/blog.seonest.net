...
<Callout title="Note">
  <p>**메타데이터 라이브러리가 여전히 필요할지도 모릅니다**</p>
  <p>
    간단한 사용 케이스에서는 Document 메타데이터를 렌더링하는 것이 적합할 수 있지만 라이브러리는
    현재 라우트 기반으로 일반적인 메타데이터를 특정 메타데이터로 재정의하는 것과 같은 보다 강력한
    기능을 제공합니다. [`react-helmet`](https://github.com/nfl/react-helmet)과 같은 라이브러리나
    프레임워크를 통해서 이러한 기능을 사용하면 보다 더 쉽게 지원할 수도 있습니다.
  </p>
</Callout>
