...
race condition이 존재했던 기존의 코드에서는 ***이후의 적당한 시점에* state를 설정**해야 했습니다. 그렇게 안하면 위에서 보듯이 기대하지 않은 결과가 나올 수 있습니다.
그러나 Suspense에서는 **state를 *즉시* 설정**합니다. 따라서 오작동하는 것이 더 어렵습니다.
