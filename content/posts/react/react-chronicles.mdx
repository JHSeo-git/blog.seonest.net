...
React 이해를 위해 React 역사와 주요 발표 자료 등을 정리해본 글입니다.
...
![Screenshot of React 16.3 life cycle](/post/react/react-chronicles/react-16.3-lifecycle.png)
...
> [[JSConfUS 2013] Tom Occhino and Jordan Walke: JS Apps at Facebook](https://www.youtube.com/watch?v=GW0rj4sNH2w) - 2013-08-06

React는 재사용 가능한 코드를 추상화하고, 애플리케이션 UI 세부 정보를 숨기는 방법을 제공함으로써 사용자 커스텀 컴포넌트를 만들 수 있도록 합니다.

JSX에서 사용하는 `<div>`와 같은 요소는 엄밀히 말하면 HTML 요소가 아닙니다. React에서 `<div>`는 DOM 요소를 나타내는 태그로 사용됩니다.
이것은 브라우저에서 실제로 `<div>` 요소를 생성하는 것이 아니라, React의 Virtual DOM(Virtual DOM)에서 해당 요소를 나타내기 위한 목적으로 사용됩니다.

따라서, `<div>`와 같은 JSX 코드는 HTML 요소와 유사한 모습을 가지지만, 엄밀히 말하면 HTML 요소가 아닌 React 컴포넌트를 나타내는 것입니다.

React 컴포넌트는 단순히 상태를 보여주는 뷰(View)를 넘어서서 상태(State)와 상호작용을 통해 일종의 상태 머신(State Machine)을 정의할 수 있도록
제공합니다.

> React 컴포넌트는 상태(state)를 가지며, 이 상태는 컴포넌트의 동작과 뷰의 표현을 결정합니다. 상태는 컴포넌트가 가진 데이터의 값들을 의미하며, 상태에 따라
> 뷰가 업데이트되고 상호작용이 이루어집니다.  
> 상태 머신은 상태와 상태 전환에 따른 동작을 정의하는 모델입니다. React 컴포넌트를 이용하여 상태 머신을 구현하면, 컴포넌트의 상태와 상태 전환에 따라 동적인
> 동작과 뷰 업데이트를 관리할 수 있습니다. 예를 들어, 상태에 따라 다른 화면을 보여주거나, 사용자 입력에 따라 상태를 전환하고 다른 동작을 수행할 수 있습니다.

컴포넌트 라이프 사이클에 대해:

1. Markup Generation: 초기 렌더링에서는 모든 요소를 순회하면서 마크업 요소를 생성, 생성된 마크업 요소에 이벤트 핸들러를 연결합니다.
2. Reconcilation: 두 번째 주요 단계는 Reconcialiation(Mutation이 아니라)을 통한 UI 업데이트 입니다.ㄴ

데이터가 변할 때마다 사용자의 UI가 변하게 만든다는 것입니다.
렌더링 호출하면서 재귀적으로 그 출력을 가져옵니다.
UI가 최신인지 확인하기 위해 차이점을 확인하고 조그마한 부분만 업데이트합니다.

### React Fiber

> [Andrew Clark: What's Next for React — ReactNext 2016](https://www.youtube.com/watch?v=aV1271hd9ew) - 2016-10-03

#### 1. React Fiber는 무엇인가요?

React Fiber는 React의 내부 재조정(Reconciliation) 알고리즘의 개선된 버전을 의미합니다.
Fiber는 React 16 버전부터 도입된 업데이트 메커니즘입니다.

기존의 React 재조정 알고리즘은 재귀적으로 동작하며, 컴포넌트 트리를 효율적으로 재조정하기 위해 재귀 호출 스택을 사용합니다.
하지만 재귀 호출 스택은 복잡한 컴포넌트 트리에서 깊이 우선 탐색을 수행하는 데에는 비효율적일 수 있습니다.

React Fiber는 이러한 문제를 해결하기 위해 재조정 알고리즘을 비동기식으로 변경하여 작업 우선순위와 중단 가능성을 도입했습니다. Fiber는 작업을 작은 단위로
분할하고 우선순위를 부여하여 렌더링 작업의 조율과 제어를 가능하게 합니다.

Fiber의 주요 목표는 다음과 같습니다:

1. 더 나은 사용자 경험: Fiber는 작업을 중단하고 다시 시작할 수 있으므로 렌더링 작업이 더 많은 시간을 소비하는 동안 사용자 인터랙션이 응답성을 유지할 수
   있습니다.
2. 우선순위 기반 렌더링: Fiber는 작업에 우선순위를 부여하여 중요한 작업을 먼저 처리하고, 렌더링 작업의 조율을 가능하게 합니다. 이를 통해 React
   애플리케이션에서 성능을 최적화할 수 있습니다.
3. 비동기 렌더링: Fiber는 렌더링 작업을 분할하여 여러 프레임에 걸쳐 처리할 수 있습니다. 이를 통해 부드러운 애니메이션 및 인터랙션을 구현할 수 있습니다.

React Fiber는 내부적으로 재조정 알고리즘을 개선하고 성능을 향상시키는데 사용되는 개념이며, React의 내부 동작에 대한 세부적인 변경사항을 포함하고 있습니다. 사용자는 React Fiber를 직접적으로 다루지 않고, React 라이브러리 자체가 Fiber를 활용하여 개발자에게 더 나은 성능과 경험을 제공합니다.

#### 2. React 재조정(Reconcilation) 과정에서 실제 DOM에 어떤 방식으로 반영되나요?

> 마법 같은 일이 있는건가요?  
> _아니요_

DOM을 변경하기 위해 React는 브라우저의 웹 API 중 하나인 `appendChild`, `removeChild`, `setAttribute`와 같은 DOM 조작 메서드를 사용합니다.
이러한 DOM 조작 메서드를 통해 요소의 추가, 수정, 삭제 등의 동작을 수행하여 실제 돔을 변경합니다.

예를 들어, `ReactDOM.render()` 함수가 호출되면 React는 변환된 Virtual DOM을 실제 DOM으로 반영하기 위해 다음과 같은 동작을 수행합니다:

- 요소 추가: 새로운 요소가 Virtual DOM에 추가되었다면, React는 `createElement`와 `appendChild`와 같은 DOM 조작 메서드를 사용하여 해당
  요소를 실제 DOM에 추가합니다.
- 요소 수정: 기존의 요소가 변경되었다면, React는 `setAttribute`와 같은 DOM 조작 메서드를 사용하여 해당 요소의 속성을 수정하거나 스타일을 변경합니다.
- 요소 제거: Virtual DOM에 존재하지 않는 요소가 이전에 실제 DOM에 있었다면, React는 `removeChild`와 같은 DOM 조작 메서드를 사용하여 해당 요소를
  실제 DOM에서 제거합니다.

위의 동작들은 React 내부 코드에서 실제 DOM을 변경하기 위해 사용되는 일부 메서드와 예시입니다. React는 이를 통해 Virtual DOM의 변경 사항을 적절히
실제 DOM에 반영하여 UI를 업데이트합니다.
따라서 실제 DOM 조작에 대한 구체적인 메서드는 브라우저의 웹 API에 따라 다를 수 있으며, React는 이를 추상화하여 개발자에게 편리한 인터페이스를 제공합니다.

### React Scheduling

> [Scheduling is the Future - Brandon Dail aka @aweary at @ReactEurope 2019](https://www.youtube.com/watch?v=Iyrf52cwxQI)

### React Fiber and Scheuding Arichitecture
...
### FB5 - Building The New Facebook With React and Relay

> [Building The New Facebook With React and Relay | Ashley Watkins](https://www.youtube.com/watch?v=KT3XKDBZW7M)

### Relay Deep Dive

> [Relay Deep Dive – Greg Hurrell](https://www.youtube.com/watch?v=oPSuvaYmXBY)

### setState Machine

> [setState Machine - Michele Bertoli](https://www.youtube.com/watch?v=sZg3DoTfHLQ)

## reference

- https://react.dev
- https://seonest.net
- https://twitter.com/KrComet/status/1558427559960154112
